(ns acceptance.cms-site
  (:use [pallet.action :only[with-action-options]]
        [pallet.configure :only[compute-service]]
        [pallet.api :only[converge]]
        [management.config :only [db-settings delivery-settings]]
        [management.groups.database :only[database-group]]
        [management.groups.delivery :only[delivery-group]]        
        [expectations]
        [clj-webdriver.taxi :only [set-driver! to click exists? input-text submit quit] :as scraper]
        [clojure.core]
        [management.models.cms-config-file))
