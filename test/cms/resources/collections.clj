...
(clutch/create! db2)
(def coll-id (:id (get-collection "test" db1)))
(get-collection-items-by-id coll-id db1)
(require 'cms.representations.items)
(require 'cms.representations.collections)
(cms.resources.common/all)
(cms.resources.common/map-from-db (keys db1)
(def test-item (first (get-collection-items "test" db1)))
(:links (cms.representations.collections/links "test"))
(require 'cheshire.core)
(keys (cheshire.core/parse-string (cms.representations.collections/render-collection (get-collection "test" db1)) true))
...
(def raw (keywordize-keys (into {} (map (juxt :_id :data) (map #(get db1 %) (map :id (clutch/get-view db1 "item" :all)))))))
(def filtered (filter #(=  coll-id (-> raw % :collection)) (keys raw)))
(map #(get db1 (name %)) filtered)
...
;;(defn db1 [] (clutch/get-database db-resource))
(def db1 (clutch/couch "gusto-cms"))
(clutch/create! db1)
(get-all-documents db1)
(get-collection-items-by-id (:id (get-collection "test2" db1)) db1) 
(get-collection-items "test2" db1)
(get-all-documents db1)
(clutch/all-databases)
(def test-id (:id (get-collection "test" db1)))
(get db1 test-id)
(def ids (map :id  (clutch/get-view db1 "item" :all)))

;; Test Business Domain Entities and Composition Heirarchy
(def user-schema {:fields {:username "foo" :type "text"
                           :passphrase "test123" :type password}
                  :childern [:domain :account :group]})

(def domain-schema {:fields {:domain-name "a.com"}
                    :childern [:landing-site :market-vector :market-matrix]})

(def landing-site-schema {:fields {:title "Patient Comfort Way"}
                     :childern [:landing-site :market-vector :market-matrix]})
...
(def user-item (create-item "user" {:username "matt" :passphrase "123test"}))
(def domain-item (create-item "domain" {:domain-name "marketwithgusto.com"}))
(def landing-site-item (create-item "landing-site" {:title "Market With Gusto"}))
...
(ancestors
 ...
 (children gather-heirarchy 'user)
                           ...
                           (derive 'advertising/keyword-bucket 'advertising/ad)
                           (derive  'advertising/keyword-bucket 'advertising/adgroup)
...
(map  #(= ((get db1 %)) ids)
      (filter #(= (:test (get-collections )) (-> (val %) :collection))   (keywordize-keys (into {} (map (juxt :_id :data )
                                                                                                        ...
                                                                                                        (into {} (get-collection-items-by-id (:id (get-collection "test" db1)) db1))
